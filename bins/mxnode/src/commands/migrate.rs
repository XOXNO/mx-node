...
pub fn run(args: MigrateFromBashArgs, global: &GlobalArgs) -> Result<(), CliError> {
    ...
    if !global.json {
        ...
        if common.nodes_scanned > 0 {
            if !common.prefs.is_empty() || !common.config.is_empty() {
                println!(
                    "  ↳ rolled up {} prefs override(s) + {} config override(s) from {} node(s)",
                    common.prefs.len(),
                    common.config.len(),
                    common.nodes_scanned,
                );
            }
            if common.differing_prefs_keys + common.differing_config_keys > 0 {
                println!(
                    "  ↳ {} key(s) differ across nodes — left in place per-node",
                    common.differing_prefs_keys + common.differing_config_keys
                );
            }
        }
    ...
    if global.json {
        let report = MigrateReport {
            ...
            common_config_overrides: common.config.len() as u32,
            differing_keys: (common.differing_prefs_keys + common.differing_config_keys) as u32,
...
fn render_sparse_config(
    ...
    let has_common_config = !common.config.is_empty();
    if has_legacy_overrides || has_common_prefs || has_common_config {
        ...
        // Fleet-wide tweaks that all bash-managed nodes had identical
        // values for collapse into a single override section so the
        // operator never has to edit N files again.
        if has_common_prefs {
        ...
        if has_common_config {
            ...
            for (k, v) in &common.config {
                config_table.insert(k.clone(), v.clone());
            }
            overrides.insert("config".to_string(), Value::Table(config_table));
        }
    ...
    if common.nodes_scanned > 0
        && (!common.prefs.is_empty() || !common.config.is_empty())
    {
        out.push_str(&format!(
            "# [overrides.prefs] / [overrides.config] were auto-derived from settings\n# identical across all {} bash-managed node(s).\n",
            common.nodes_scanned
        ));
    }
    out.push('\n');
