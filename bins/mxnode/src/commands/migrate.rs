...
//!   * GITHUBTOKEN is never written to config.toml. Per the global
//!     CLAUDE.md rule "secrets do not belong in config files", we
//!     surface it back to the operator with shell-export instructions.
//!     The token is partially masked (first 4 + tail length) in dry-run
//!     output.
...
pub fn run(args: MigrateBashArgs, global: &GlobalArgs) -> Result<(), CliError> {
    ...
    // ── apply config.toml patches (best-effort, non-fatal) ──
    let config_changes = if !plan.config_patches.is_empty() || !plan.per_node_patches.is_empty() {
        match apply_config_patches(&plan.config_patches, &plan.per_node_patches) {
            Ok(path) => Some(path),
            Err(e) => {
...
/// Merge config patches into the user's `config.toml`. Comments and
/// section ordering are preserved by routing through
/// `toml_edit::DocumentMut`, same as `mxnode config set`.
fn apply_config_patches(
    ...
    let body = if target.exists() {
        ...
        "# mxnode config — generated by `mxnode migrate-bash`.\nschema_version = 1\n".to_string()
    };
...
fn print_dry_run(plan: &MigrationPlan, global: &GlobalArgs) {
    ...
    if let Some(token) = &plan.github_token {
        println!();
        println!("found GITHUBTOKEN in variables.cfg (NOT written to config.toml):");
        println!("  {}", mask_token(token));
        println!("  to use it, export in your shell:");
        println!("    export MXNODE_GITHUB_TOKEN='<your token>'");
    }
...
fn print_apply_summary(
    ...
    if let Some(token) = &plan.github_token {
        println!();
        println!("found GITHUBTOKEN — NOT written to disk:");
        println!("  {}", mask_token(token));
        println!("  export it in your shell to use:");
        println!("    export MXNODE_GITHUB_TOKEN='<your token>'");
    }
    if !plan.warnings.is_empty() {
        println!();
