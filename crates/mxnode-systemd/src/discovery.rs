...
mod tests {
    use super::*;
    use crate::render::{render_canonical_node_unit, render_legacy_node_unit, NodeUnitSpec};
    use mxnode_core::NodeIndex;
    ...
    #[test]
    fn parses_legacy_indented_node_unit() {
        let workdir = PathBuf::from("/home/ubuntu/elrond-nodes/node-0");
        let unit = render_legacy_node_unit(&spec_node_0(&workdir));
        let view = parse_unit_text(&unit).unwrap();
        // Legacy form has leading "  " on every line; our parser must
        // trim that and still recover the same fields.
        assert_eq!(view.user.as_deref(), Some("ubuntu"));
        assert_eq!(view.working_directory, Some(workdir));
        assert_eq!(view.api_port, Some(8080));
    }

    /// Canonical and legacy forms differ on whitespace but must yield the
    /// *same* `UnitView` — that's what makes byte-different units adoptable
    /// without "drift".
    #[test]
    fn canonical_and_legacy_yield_equal_views() {
        let workdir = PathBuf::from("/home/ubuntu/elrond-nodes/node-0");
        let v_legacy = parse_unit_text(&render_legacy_node_unit(&spec_node_0(&workdir))).unwrap();
        let v_canon = parse_unit_text(&render_canonical_node_unit(&spec_node_0(&workdir))).unwrap();
        assert_eq!(v_legacy, v_canon);
    }
