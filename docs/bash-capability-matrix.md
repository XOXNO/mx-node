...
| `add_nodes` | Covered | `mxnode add-nodes --count N`; `bins/mxnode/src/commands/add_nodes.rs` | Rejected for squad installs, matching Bash's safety rule. |
...
| `cleanup` | Covered and safer by default | `mxnode cleanup --yes --execute`; dry-run default | Removes managed units/files while preserving the Bash command's explicit confirmation intent. |
...
| `get_logs` | Covered and extended | `mxnode logs --save-archive`; live logs via `mxnode logs -f`, `mxnode logs --ws`, and dashboard | Archive destination follows managed paths; `--ws --log-save` covers logviewer-style live saves. |
| `benchmark` | Covered | `mxnode benchmark` | Still depends on bundled assessment tooling availability. |
...
| TermUI | Covered and extended | `mxnode dashboard` | Rust TUI replaces upstream termui and adds multi-node view. |
| Logviewer | Covered and extended | `mxnode logs --ws --node N --log-level '*:DEBUG' --log-save`; `mxnode dashboard --ws-logs` | Reuses the node `/log` WebSocket protocol and custom profile JSON fields from the upstream Go logviewer; dashboard keeps the multi-node view. |
| Keygenerator utility | Covered | `mxnode keygen --output DIR`; install/upgrade refresh keygenerator artifacts | Preserves the utility wrapper without requiring users to run binaries out of `elrond-utils` manually. |
...
| Migrate existing Bash install | Covered | `mxnode migrate-bash` | Imports marker files, variables, units, and proxy shape. |
...
| Seednode utility | Covered in this pass | `mxnode install` installs `elrond-utils/seednode/{seednode,config/*}`; `mxnode upgrade` refreshes it; `mxnode seednode -- ...` runs it | Copies `config.toml` and intended `p2p.toml` from the config repo when present. |
...
| System requirements | Covered with caveats | `mxnode doctor` checks CPU/RAM/disk scaled by node count, CPU flags/ARM caveats, OS floor, p2p ports; `mxnode install` gates planned installs before tag resolution unless `--skip-safety-checks` is passed | Dedicated physical CPU and NAT/UPnP status still require operator/network verification. |
